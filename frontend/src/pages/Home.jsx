        ...
        <div className="grid md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <div className="flex flex-wrap gap-3 text-xs">
              <button onClick={() => document.getElementById('vehicles')?.scrollIntoView({ behavior: 'smooth' })} className="px-4 py-2 rounded-xl bg-primary text-white font-medium hover:bg-primary/90">
                Browse Vehicles
              </button>
              <button onClick={() => document.getElementById('vendors')?.scrollIntoView({ behavior: 'smooth' })} className="px-4 py-2 rounded-xl border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:border-primary">
                View Garages
              </button>
            </div>
          </div>
            ...
            filtered.map(v => (
              <article key={v.id} className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 flex flex-col justify-between shadow-sm hover:shadow-md transition">
                ...
                <div className="flex items-center gap-2 text-[11px]">
                  <button
                    onClick={() => navigate(`/booking?id=${encodeURIComponent(v.id)}`)}
