...
export default function Register() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true)
    try {
      const res = await apiPost('/auth/register', { fullName, email, password })
      if (res.ok) {
        navigate('/login')
      } else {
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-darkbg flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-6">
          ...
          <h1 className="mt-3 text-xl font-bold dark:text-white">Create account</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Start renting bikes today</p>
        </div>
